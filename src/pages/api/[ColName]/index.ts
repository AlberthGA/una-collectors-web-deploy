...
const getAll = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const documents = await dbcontext.collection(ColName as string).find({}).toArray();
...
const create = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    (result.acknowledged) ? res.status(200).json(document) : res.status(500).send('No se pudo insertar el documento');
}
